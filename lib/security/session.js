 ...
 */
exports.create = function ( id, data, modelName ) {
...
exports.get = function ( token, modelName ) {
  return new Promise(function ( resolve, reject ) {
    ...
    SessionModel.findOne({ publicKey: token }).populate('user').exec(function ( err, session ) {
      if( err ) {
        return reject( err );
      }
      ...
      User.populate(session.user, { path: 'permissions' }, function ( err, populatedUser ) {
        if( err ) {
          return reject( err );
        }
