...
var ftp = ( process.env.environment === 'test' ) ? {} : require('../../../config/ftp');
...
Mapper.prototype._drain = function ( callback ) {
  ...
  async.mapSeries(queue, (fileObject, cb) => {
    // Freshness, FileStreamer, Callback
    fileObject.fetch(this.options.freshness, this.fileStreamer, err => {
      if( err ) {
        throw new Error( err );
      }
