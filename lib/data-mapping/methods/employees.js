...
function gotMedicalRate ( err, medicalRate ) {
  ...
  return CompanyLocation.findOneAndUpdate({ ebmsNumber: self.ebmsNumber }, { $addToSet: { employees: this.employeeId } }, function ( err ) {
    return Company.findByIdAndUpdate(medicalRate.company._id, { $addToSet: { employees: this.employeeId } }, function ( err ) {
