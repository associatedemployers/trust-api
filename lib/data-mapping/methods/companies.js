...
var MedicalRate = require(process.cwd() + '/models/medical-rate'),
...
exports.inject = function ( hashes, callback ) {
  ...
  hashes.forEach(function ( hash ) {
    Company.findOne({ name: hash.name }, function ( err, record ) {
      ...
      if( !record ) {
        var record = new MedicalRate( hash );
      } else {
        _.merge( record, hash );
      }

      record.save(function ( err, rate ) {
        if( err ) {
          winston.log('error', chalk.bgRed(err));
          throw new Error(err);
        }

        winston.info('Saved rate', rate.name.rate);
