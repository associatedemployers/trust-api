...
var DentalRate  = require(locdir + '/models/dental-rate'),
  ...
  hashes.forEach(function ( hash ) {
    ...
    DentalRate.findOne({ planNumber: hash.planNumber }, function ( err, rate ) {
      ...
      CompanyPlan.find({ legacyPlanNumber: hash.planNumber }).populate('company').exec(function ( err, linkers ) {
        ...
        linkers.forEach(function ( linker, index ) {

          if( !linker.company ) {
            delete linkers[ index ];
            return;
          }
        ...
        rate.save(function ( err, rateRecord ) {
          ...
          linkers.forEach(function ( linker ) {
            ...
            if( indexOfId( linker.company.dentalRates, rateRecord._id ) < 0 ) {
              linker.company.dentalRates.push( rateRecord._id );
