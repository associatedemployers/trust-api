...
var relationshipMap = {
  ...
  LegalDep: 'Legal Dependent'
};

var planTypes = [ 'medical', 'dental', 'vision', 'life' ];
...
exports.normalize = function ( hashes ) {
  return munger(definition, hashes).map(function ( hash ) {
    var rel = relationshipMap[ hash.relationship ];

    if ( rel ) {
      hash.relationship = rel;
    }
  ...
  return new Promise(function ( resolve, reject ) {
      ...
      var promises = dependents.map(function ( dependent ) {
        ...
        if( employee.dependents && _.isArray( employee.dependents ) ) {
          record = _.filter( employee.dependents, { legacyId: dependent.legacyId.toString() } )[0];
        }
        ...
        planTypes.forEach(function ( type ) {
          var plan          = employee.plans[ type ][ 0 ],
              coverageLevel = employee.planOptions[ type ].covers;
          ...
          if (
            coverageLevel === 'employeeAndFamily' ||
            ( coverageLevel === 'employeeAndSpouse' && dependent.relationship !== 'Child' && dependent.relationship !== 'Legal Dependent' )
          ) {
            record.plans[ type ].push( plan );
          }
        });
        ...
        return new Promise(function ( resolve, reject ) {
          record.save(function ( err, doc ) {
            ...
            if( indexOfId( employeePlain.dependents, doc._id ) < 0 ) {
