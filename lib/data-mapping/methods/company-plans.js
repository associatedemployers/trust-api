...
exports.inject = function ( hashes, callback ) {
  ...
  hashes.forEach(function ( hash ) {
    CompanyPlan.findOne({ legacyEbmsNumber: hash.legacyEbmsNumber }, function ( err, record ) {
      ...
      CompanyLocation.findOne({ ebmsNumbers: { $in: [ hash.legacyEbmsNumber ] } }, function ( err, location ) {
        if( err ) {
          winston.log('error', chalk.bgRed(err));
          throw new Error(err);
        }
        ...
        if( !location ) {
          fulfilled++;
          return winston.log('warning', 'No location for CompanyPlan', hash.legacyEbmsNumber);
        }
        ...
        record.save(function ( err, plan ) {
          ...
          winston.info( chalk.dim(fulfilled, '/', hashLen, '-', 'Saved CompanyPlan for plan #', plan.legacyPlanNumber) );
