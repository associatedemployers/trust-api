...
function Mailman ( options ) {
  ...
  this.sender = {
    from: ( options.sender && options.sender.name && options.sender.email ) ? options.sender.name + ' <' + options.sender.email + '>' : 'AE Trust <trust@aehr.org>'
  };
...
Mailman.prototype.send = function ( to, subject, templateName, vars ) {
  ...
  return new Promise(function ( resolve, reject ) {
    ...
    return self.__render(templateName, vars).then(function ( rendered ) {
      ...
      winston.log('debug', chalk.dim('Mailman :: Using auth', JSON.stringify( self.__transportConfig.auth )));
