...
Ticker.prototype.tick = function ( updated, previous, updater, options ) {
  ...
  winston.log('info', chalk.dim('Running Ticker.tick...'));
  ...
  return new Promise(function ( resolve, reject ) {
    ...
    var compDescript = ( mongoose.Types.ObjectId.isValid( updater ) ) ? 'user ' + updater.toString() : updater;  // Computed Description Part
...
Ticker.prototype.attach = function ( schema ) {
  ...
  schema.post('save', function ( doc ) {
    ...
    winston.log('info', chalk.dim( 'Running post save w/ Ticker' ));

    var Model = dbConnection.model( this.constructor.modelName );

    winston.log('info', chalk.dim( 'Working w/', this.constructor.modelName ));

    Model.findById(doc._id, previousLookup);
